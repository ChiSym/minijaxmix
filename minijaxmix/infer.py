...
def infer(key, data, n_splits, n_gibbs, categorical_idxs, n_categories, n_branch=2, rejuvenation=True, minibatch_size=1000):
    N = data.shape[0]
    logp_x_y = jnp.log(jnp.ones((N, 1)) / N)
    p_ys = jnp.array([1.])
    ws = jnp.array([jnp.mean(data, axis=0)])
    conditional_entropies = jnp.array([entropy(data)])
    jax.debug.print("initial entropy: {conditional_entropies}", conditional_entropies=conditional_entropies[0])


    def infer_step(step_key, i, logp_x_y, p_ys, ws, conditional_entropies):
        key1, key2 = jax.random.split(step_key)
        keys = jax.random.split(key2, 1 + i)
        p_x_y = jnp.exp(logp_x_y - jax.nn.logsumexp(logp_x_y, axis=0))
        minibatches = jax.vmap(jax.random.choice, in_axes=(0, None, None, None, 0))(keys, data, (minibatch_size,), True, p_x_y.T)
        ...
        conditional_entropies = jnp.concatenate((conditional_entropies, cluster_entropies[best_idx]), axis=0)

        prev_p_y = p_ys[best_idx]
        p_ys = jnp.delete(p_ys, best_idx, axis=0, assume_unique_indices=True)
        p_ys = jnp.concatenate((p_ys, prev_p_y * cluster_p_ys[best_idx]), axis=0)

        ws = jnp.delete(ws, best_idx, axis=0, assume_unique_indices=True)
        ws = jnp.concatenate((ws, cluster_ws[best_idx]), axis=0)
        ...
        return (logp_x_y, p_ys, ws, conditional_entropies), None
    ...
    for i in range(n_splits):
        key = keys[i]
        (logp_x_y, p_ys, ws, conditional_entropies), _ = infer_step(key, i, logp_x_y, p_ys, ws, conditional_entropies)

    return logp_x_y, p_ys, ws, conditional_entropies
