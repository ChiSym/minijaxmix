...
def predictive_logprob(data, w, logp_y):
    logprobs = jax.vmap(jax.vmap(logprob, in_axes=(None, 0)), in_axes=(0, None))(data, w)
    logprobs = jax.nn.logsumexp(logprobs + logp_y, axis=1)
...
def infer(key, data, categorical_idxs, n_iters, proposals_per_particle, n_gibbs, n_categories, rejuvenation=True, minibatch_size=1000, test=False, test_data=None):
    ...
    logp_ys = -jnp.inf * jnp.ones(n_clusters)
    logp_ys = logp_ys.at[0].set(0.)
    ...
    def infer_step(carry, key_i):
        logp_y, w, conditional_H = carry
        step_key, i = key_i
        logp_x_y = update_logp_x_y(data, w)
        logp_y_x = update_logp_y_x(logp_y, logp_x_y)

        # choose which clusters to split
        key1, key2 = jax.random.split(step_key)
        clusters_to_split = stratified_resampling(key1, logp_y, proposals_per_particle)
        ...
        cluster_logp_ys, cluster_ws, cluster_H, total_H = jax.vmap(split_proposal, in_axes=(0, 0, None, None, None, None))(
            keys, minibatches, n_gibbs, categorical_idxs, n_categories, 2)

        H_deltas = jnp.exp(logp_y[clusters_to_split]) * (conditional_H[clusters_to_split] - total_H)
        best_idx = jnp.nanargmax(H_deltas)
        cluster_idx = clusters_to_split[best_idx]

        best_logp_y = cluster_logp_ys[best_idx]
        best_w = cluster_ws[best_idx]
        best_H = cluster_H[best_idx]

        prev_logp_y = logp_y[cluster_idx]
        logp_y = logp_y.at[cluster_idx].set(prev_logp_y + best_logp_y[0])
        logp_y = logp_y.at[i+1].set(prev_logp_y + best_logp_y[1])
        ...
        total_H_split = jnp.nansum(conditional_H * jnp.exp(logp_y)) - jnp.nansum(jnp.exp(logp_y) * jnp.log(jnp.exp(logp_y)))
        ...
        if test:
            ...
            logprobs = jax.nn.logsumexp(logprobs + logp_y, axis=1)
            logprobs = jnp.mean(logprobs)
            return (logp_y, w, conditional_H), (total_H_split, logprobs)
        else:
            return (logp_y, w, conditional_H), (total_H_split, None)
    ...
    def rejuvenation(carry, key):
        ...
        def rejuvenation_step(logp_y_w, key):
            return gibbs_sampling(key, data, logp_y_w[0], logp_y_w[1], categorical_idxs, n_categories), None

        keys = jax.random.split(key, n_gibbs)

        (logp_y, w), _ = jax.lax.scan(rejuvenation_step, (logp_y, w), keys)
        logp_x_y = update_logp_x_y(data, w)
        logp_y_x = update_logp_y_x(logp_y, logp_x_y)

        conditional_H = conditional_entropy(data, jnp.exp(logp_y_x))
        total_H_rejuvenation = jnp.nansum(conditional_H * jnp.exp(logp_y)) - jnp.nansum(jnp.exp(logp_y) * jnp.log(jnp.exp(logp_y)))

        return (logp_y, w, conditional_H), total_H_rejuvenation

    key, subkey = jax.random.split(key)
    keys = jax.random.split(subkey, n_clusters - 1)

    (logp_ys, ws, conditional_H), (total_H_split, logprobs) = jax.lax.scan(
        infer_step, (logp_ys, ws, conditional_H), (keys, jnp.arange(n_clusters - 1)))
    ...
    return logp_ys, ws, conditional_H, total_H_split, total_H_rejuvenation, logprobs
...
def split_proposal(key, data, n_gibbs, categorical_idxs: Integer[Array, "k"], n_categories: int, n_segments=2):
    ...
    logp_y_x = jnp.log(jax.vmap(jax.random.dirichlet, in_axes=(0, None))(keys, alpha))
    logp_y = update_logp_y(logp_y_x)
    key, subkey = jax.random.split(key)
    w = update_w(subkey, data, logp_y_x, categorical_idxs, n_categories)

    @jax.profiler.annotate_function
    def em_step(logp_y_w, key):
        logp_y, w = gibbs_sampling(key, data, logp_y_w[0], logp_y_w[1], categorical_idxs, n_categories)
        return (logp_y, w), (logp_y, w)

    key, subkey = jax.random.split(key)
    keys = jax.random.split(subkey, n_gibbs)
    (logp_y, w), (logp_ys, ws) = jax.lax.scan(em_step, (logp_y, w), keys)

    logp_x_y = update_logp_x_y(data, w)
    logp_y_x = update_logp_y_x(logp_y, logp_x_y)

    cluster_H = conditional_entropy(data, jnp.exp(logp_y_x))
    total_H = cluster_H @ jnp.exp(logp_y) - jnp.sum(jnp.exp(logp_y) * jnp.log(jnp.exp(logp_y)))

    return logp_y, w, cluster_H, total_H
...
def update_logp_y_x(logp_y: Float[Array, 'c'], logp_x_y: Float[Array, 'n c']):
    logp_y_x0 =  logp_y[None, :] + logp_x_y
...
def update_w(key: Array, data: Bool[Array, 'n k'], logp_y_x: Float[Array, 'n c'], categorical_idxs: Integer[Array, "k"], n_categories: int):
    ...
    w_new = jax.vmap(sample_dirichlet, in_axes=(0, 0, None, None))(keys, alpha, categorical_idxs, n_categories)

    return w_new

@jax.named_scope("update_logp_y")
def update_logp_y(logp_y_x: Float[Array, 'n c']):
    logp_y = jax.nn.logsumexp(logp_y_x, axis=0) - jnp.log(logp_y_x.shape[0])
    return logp_y

@partial(jax.jit, static_argnames=("n_categories"))
@jax.named_scope("gibbs_sampling")
def gibbs_sampling(key: Array, data: Bool[Array, 'n k'], logp_y: Float[Array, 'c'], w_init: Float[Array, 'c k'], categorical_idxs: Integer[Array, "k"], n_categories: int):
    ...
    anynan_logp_x_y = jnp.isnan(logp_x_y).any()
    anynan_logp_y_x = jnp.isnan(logp_y_x).any()
    anynan_logp_y = jnp.isnan(logp_y).any()
    anynan_w = jnp.isnan(w_new).any()

    anyinf_logp_x_y = jnp.isinf(logp_x_y).any()
    anyinf_logp_y_x = jnp.isinf(logp_y_x).any()
    anyinf_logp_y = jnp.isinf(logp_y).any()
    anyinf_w = jnp.isinf(w_new).any()
